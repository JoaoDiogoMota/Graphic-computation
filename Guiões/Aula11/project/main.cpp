...
GLuint vertexCount, vertices, normals, texCoord, indices, indCount, texIDCylinder, texIDFloor;

GLuint* textureBuff = new GLuint [2];
GLuint* textureCoords = new GLuint [2];
...
void prepareCylinder(float altura, float raio, int lados) {
	...
	glGenBuffers(1, &textureCoords[0]);
	glBindBuffer(GL_ARRAY_BUFFER, textureCoords[0]);
...
void drawFloor(float xmin, float xmax, float zmin, float zmax) {
	...
	glBindTexture(GL_TEXTURE_2D, textureBuff[1]);
...
void drawCylinder() {
	...
	glBindTexture(GL_TEXTURE_2D, textureBuff[0]);
	glBindBuffer(GL_ARRAY_BUFFER, textureCoords[0]);
...
void loadTexture(int position, std::string s) {
	...
	glGenTextures(1, &textureBuff[position]);

	glBindTexture(GL_TEXTURE_2D, textureBuff[position]);
...
void prepareTextures() {
	loadTexture(0, "Oil_Drum001h.jpg");
	loadTexture(1, "Concrete.jpg");
}
void initGL() {
